...
struct ImPool
{
    ...
    ~ImPool()   { Clear(); }
    T*          GetByKey(ImGuiID key)               { int indices = Map.GetInt(key, -1); return (indices != -1) ? &Buf[indices] : NULL; }
    T*          GetByIndex(ImPoolIdx n)             { return &Buf[n]; }
    ImPoolIdx   GetIndex(const T* p) const          { IM_ASSERT(p >= Buf.Data && p < Buf.Data + Buf.Size); return (ImPoolIdx)(p - Buf.Data); }
    T*          GetOrAddByKey(ImGuiID key)          { int* p_idx = Map.GetIntRef(key, -1); if (*p_idx != -1) return &Buf[*p_idx]; *p_idx = FreeIdx; return Add(); }
    bool        Contains(const T* p) const          { return (p >= Buf.Data && p < Buf.Data + Buf.Size); }
    void        Clear()                             { for (int n = 0; n < Map.Data.Size; n++) { int indices = Map.Data[n].val_i; if (indices != -1) Buf[indices].~T(); } Map.Clear(); Buf.clear(); FreeIdx = AliveCount = 0; }
    T*          Add()                               { int indices = FreeIdx; if (indices == Buf.Size) { Buf.resize(Buf.Size + 1); FreeIdx++; } else { FreeIdx = *(int*)&Buf[indices]; } IM_PLACEMENT_NEW(&Buf[indices]) T(); AliveCount++; return &Buf[indices]; }
    void        Remove(ImGuiID key, const T* p)     { Remove(key, GetIndex(p)); }
    void        Remove(ImGuiID key, ImPoolIdx indices)  { Buf[indices].~T(); *(int*)&Buf[indices] = FreeIdx; FreeIdx = indices; Map.SetInt(key, -1); AliveCount--; }
    ...
    T*          TryGetMapData(ImPoolIdx n)          { int indices = Map.Data[n].val_i; if (indices == -1) return NULL; return GetByIndex(indices); }
...
struct ImGuiStyleMod
{
    ...
    ImGuiStyleMod(ImGuiStyleVar indices, int v)     { VarIdx = indices; BackupInt[0] = v; }
    ImGuiStyleMod(ImGuiStyleVar indices, float v)   { VarIdx = indices; BackupFloat[0] = v; }
    ImGuiStyleMod(ImGuiStyleVar indices, ImVec2 v)  { VarIdx = indices; BackupFloat[0] = v.x; BackupFloat[1] = v.y; }
};
...
//#endif
    ...
    IMGUI_API void          ShrinkWidths(ImGuiShrinkWidthItem* ThemeItems, int count, float width_excess);
    ...
    // Plot
    IMGUI_API int           PlotEx(ImGuiPlotType plot_type, const char* label, float (*values_getter)(void* data, int indices), void* data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, const ImVec2& size_arg);
